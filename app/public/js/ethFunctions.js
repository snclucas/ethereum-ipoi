...
function getLatestInfo() {
    try {
        ipoiContract = web3.eth.contract(JSON.parse(document.getElementById("ABI").value)).at(document.getElementById("contract").value);
...
function checkOnHudsonsMarriage() {
    try {
        getLatestInfo();
        //var vacationFund = web3.eth.getBalance(contractAddress);
        var ipoiDateRaw = ipoiContract.marriageDate({ from: web3.eth.coinbase });
        var marriageDate = moment(parseInt(marriageDateRaw, 10) * 1000).format('MMMM Do YYYY, h:mm:ss a');
        console.log(marriageDate);

        document.getElementById("mStatus").value = web3.toAscii(ipoiContract.marriageStatus({ from: web3.eth.coinbase }));
        document.getElementById("partner1").value = web3.toAscii(ipoiContract.partner1({ from: web3.eth.coinbase }));
        document.getElementById("partner2").value = web3.toAscii(ipoiContract.partner2({ from: web3.eth.coinbase }));
        document.getElementById("mDate").value = marriageDate;
        marriageImageHash = web3.toAscii(ipoiContract.imageHash({ from: web3.eth.coinbase }));
        document.getElementById("mImage").innerHTML = "<a href=https://ipfs.pics/" + marriageImageHash + " &quot; target=&quot;_blank&quot;>" + marriageImageHash + "</a>";
        marriageProofHash = web3.toAscii(ipoiContract.marriageProofDoc({ from: web3.eth.coinbase }));
        document.getElementById("mProof").innerHTML = "<a href=https://ipfs.pics/" + marriageProofHash + " &quot; target=&quot;_blank&quot;>" + marriageProofHash + "</a>";
...
function getMajorEvents() {
	...
	var event = ipoiContract.MajorEvent( {}, {fromBlock: 0, toBlock: 'latest'});
